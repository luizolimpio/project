...
def login_pageinicial(request, template_name="lfs/customer/login_pageinicial.html"):
    """Custom view to login or register/login a user.
    ...
    if request.POST.get("action") == "login":
        ...
        if login_form.is_valid():
            redirect_to = request.POST.get("next")
            # Light security check -- make sure redirect_to isn't garbage.
            if not redirect_to or '//' in redirect_to or ' ' in redirect_to or reverse(
                    "lfs_login_pageinicial") in redirect_to or reverse(
                "password_reset_complete") in redirect_to or reverse("lfs_password_reset") in redirect_to:
                redirect_to = reverse("lfs_shop_home")
    ...
    if next_url is None:
        next_url = request.META.get("HTTP_REFERER")
    if next_url is None:
        next_url = reverse("lfs_shop_home")
...
def logout(request):
    ...
    logout(request)
    redirect_to = reverse("lfs_login_pageinicial")
    return lfs.core.utils.set_message_cookie(
        redirect_to, msg=_(u"You have been logged out."))
